const fs = require('fs');
const path = './data'; // JSON dosyalarının yer alacağı dizin

// Dosyadaki verileri okuma fonksiyonu
function readData(filename) {
    try {
        return JSON.parse(fs.readFileSync(`${path}/${filename}.json`, 'utf8'));
    } catch (error) {
        return []; // Eğer dosya yoksa veya okuma hatası varsa, boş array döner.
    }
}
...
// Veri yapıları - JSON'dan verileri okuma
let users = readData('users');
let comments = readData('comments');
let sponsors = readData('sponsors');
let currentUser = readData('currentUser') || null;
...
// Başlangıç verilerini kontrol et ve dosyaya yaz
...
function updateUserSettings() {
    ...
    // Ad ve email zorunlu alanlar
    if (!name || !email) {
        alert('Ad ve email alanları zorunludur!');
        return;
    }

    // Eğer email değişmediyse, kontrol yapmaya gerek yok
    ...
    if (userIndex !== -1) {
        ...
        // Şifreyi sadece kullanıcı değiştirirse güncelle
        if (password) {
...
function deleteCurrentUser() {
    ...
    // Kullanıcıyı ve onun yorumlarını sil
    users = users.filter(u => u.id !== currentUser.id);
    comments = comments.filter(c => c.userId !== currentUser.id);
    // Eğer sponsor verisi varsa, sponsorları da sil
...
function handleDeleteUser(e) {
    ...
    if (confirm('Bu kullanıcıyı silmek istediğinize emin misiniz?')) {
        ...
        sponsors = sponsors.filter(s => s.userId !== userId);
...
function setupEventListeners() {
    ...
    // Sayfa yenilendiğinde verileri kaydet
